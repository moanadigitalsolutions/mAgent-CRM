...
class Command(BaseCommand):
    help = 'Create sample data for the mAgent CRM'

    def handle(self, *args, **options):
        self.stdout.write('Creating sample data for mAgent CRM...')
        ...
        for field_data in custom_fields_data:
            ...
            if created:
                ...
                self.stdout.write(f"- Custom field already exists: {custom_field.label}")
            ...
            try:
                ...
                for sample in sample_values:
                    ...
                    for field, value in sample['values'].items():
                        ...
                        if created:
                            self.stdout.write(f"✓ Added {field.label} for {customer.full_name}: {value}")
            except CustomField.DoesNotExist:
                self.stdout.write("⚠ Some custom fields not found, skipping custom values")

        self.stdout.write(self.style.SUCCESS('\n✅ Sample data creation completed!'))
        self.stdout.write('\nYou can now:')
        self.stdout.write('1. Visit http://127.0.0.1:8000/ to see the dashboard')
        self.stdout.write('2. Visit http://127.0.0.1:8000/customers/ to see the customer list')
        self.stdout.write('3. Visit http://127.0.0.1:8000/admin/ to access the admin panel')
        self.stdout.write('   Username: admin')
        self.stdout.write('   Password: [the password you set during superuser creation]')